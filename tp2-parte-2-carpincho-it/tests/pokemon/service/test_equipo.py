...
from app.pokemon.exceptions import InvalidPokemonMovimiento
...
class TestEquipoService:
    def test_create_equipo(self, session: Session):
        data = EquipoUpsert(nombre="Rocket", id_generacion=1)
        equipo = EquipoService(session).create_equipo(data)
        assert equipo.id == 1
        assert equipo.generacion.id == 1
        assert equipo.nombre == "Rocket"

    def test_exception_create_equipo_duplicate_name(self, session: Session):
        data = EquipoUpsert(nombre="Rocket", id_generacion=1)
        EquipoService(session).create_equipo(data)
        with pytest.raises(HTTPException) as exc_info:
            EquipoService(session).create_equipo(data)

        assert "El nombre de equipo ya fue registrado" in str(exc_info.value)

    def test_get_equipo(self, session: Session):
    ...
    def test_exception_get_equipo(self, session: Session):
        with pytest.raises(HTTPException) as exc_info:
            EquipoService(session).get_equipo_por_id(99999999999)

        assert "Equipo no encontrado" in str(exc_info.value)

    def test_get_equipos(self, session: Session):
        service = EquipoService(session)
        ...
        for equipo in equipos:
            assert isinstance(equipo, EquipoResumidoPublic)

    def test_update_equipo(self, session: Session):
        service = EquipoService(session)
        data1 = EquipoUpsert(nombre="Rocket", id_generacion=1)
        data2 = EquipoUpsert(nombre="Rocket2", id_generacion=2)
        service.create_equipo(data1)
    ...
    def test_delete_equipo(self, session: Session):
        service = EquipoService(session)
        data1 = EquipoUpsert(nombre="Rocket", id_generacion=1)
        service.create_equipo(data1)
    ...
    def test_add_integrante(self, session: Session):
        service = EquipoService(session)
        data1 = EquipoUpsert(nombre="Rocket", id_generacion=1)
        service.create_equipo(data1)
    ...
    def test_add_integrante_invalid_pokemon(self, session: Session):
        service = EquipoService(session)
        data1 = EquipoUpsert(nombre="Rocket", id_generacion=1)
        service.create_equipo(data1)
    ...
    def test_add_integrante_invalid_generacion(self, session: Session):
        service = EquipoService(session)
        data1 = EquipoUpsert(nombre="Rocket", id_generacion=1)
        service.create_equipo(data1)

        data2 = EquipoIntegranteAdd(apodo="waxy", id_pokemon=607)
    ...
    def test_get_integrante(self, session: Session):
        service = EquipoService(session)
        service.create_equipo(EquipoUpsert(nombre="Rocket", id_generacion=5))
        service.add_integrante(1, EquipoIntegranteAdd(id_pokemon=607, apodo="waxy"))
    ...
    def test_add_movimiento_integrante(self, session: Session):
        service = EquipoService(session)
        service.create_equipo(EquipoUpsert(nombre="Rocket", id_generacion=5))
        service.add_integrante(1, EquipoIntegranteAdd(apodo="waxy", id_pokemon=607))

        movimiento = service.add_movimiento_a_integrante(1, 1, 114)
        assert movimiento.id == 114

        integrante = service.get_integrante(1, 1)
        assert len(integrante.movimientos) == 1
        assert integrante.movimientos[0].id == 114

    def test_add_invalid_movimiento_integrante(self, session: Session):
        service = EquipoService(session)
        service.create_equipo(EquipoUpsert(nombre="Rocket", id_generacion=5))
        service.add_integrante(1, EquipoIntegranteAdd(apodo="waxy", id_pokemon=607))

        with pytest.raises(HTTPException) as exc_info:
            service.add_movimiento_a_integrante(1, 1, 1)

        assert "El movimiento no pertence al pokemon" in str(exc_info.value)
